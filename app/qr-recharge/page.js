...
function RechargePage(){
 ...
 const [user,setUser]=useState(null),[payment,setPayment]=useState({qrUrl:""}),[utr,setUtr]=useState(""),[requests,setRequests]=useState([]),[msg,setMsg]=useState(""),[saving,setSaving]=useState(false),[sent,setSent]=useState(false);
 useEffect(()=>onAuthStateChanged(auth,setUser),[]);
 useEffect(()=>{getDoc(doc(db,"settings","payment")).then(s=>s.exists()&&setPayment({qrUrl:s.data().qrUrl||""})).catch(()=>{})},[]);
 ...
 <section className="cardPage" style={{maxWidth:760,margin:"25px auto"}}><div style={{textAlign:"center"}}><div className="paymentIcon">₹</div><span className="eyebrow">DIRECT QR PAYMENT</span><h1>₹{amount} Access Payment</h1><p>Profile <b>{profile}</b> ke liye exact ₹{type==="mobile"?500:100} payment karein.</p></div>
 ...
 <div className="qr" style={{textAlign:"center"}}>{payment.qrUrl?<><img src={payment.qrUrl} alt="UPI QR" style={{maxWidth:300,width:"100%",borderRadius:12}}/><p><b>QR scan karke UPI app me ₹{type==="mobile"?500:100} enter karein.</b></p></>:<small>Admin ne QR set nahi kiya.</small>}</div>
