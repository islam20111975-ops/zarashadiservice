...
function PageBody(){
 ...
 const [user,setUser]=useState(null),[upiId,setUpiId]=useState(""),[utr,setUtr]=useState(""),[requests,setRequests]=useState([]),[msg,setMsg]=useState(""),[saving,setSaving]=useState(false),[sent,setSent]=useState(false);
 useEffect(()=>onAuthStateChanged(auth,setUser),[]);
 useEffect(()=>getDoc(doc(db,"settings","payment")).then(s=>s.exists()&&setUpiId(s.data().upiId||"")).catch(()=>{}),[]);
 ...
 async function submit(e){
  ...
  try{
   ...
   batch.set(reqRef,{uid:user.uid,profileId:profile,type,amount,status:"pending",paymentMethod:"upi",utr:clean,utrClaimId:claimRef.id,lockId,createdAt:serverTimestamp()});
