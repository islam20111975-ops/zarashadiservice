...
export default function Account(){
  ...
  return <main><section className="cardPage" style={{maxWidth:760,margin:"25px auto"}}>
    ...
    <div className="adminBox"><div className="boxTitle"><div><span className="eyebrow">WALLET</span><h3>💰 Wallet Balance: ₹{wallet}</h3></div></div><p className="small">₹100 / ₹500 / ₹1000 में से amount चुनें। UPI button पर क्लिक करने पर वही amount अपने-आप UPI payment में जाएगा.</p><div className="rechargeOptions" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,margin:"12px 0"}}>{RECHARGE_OPTIONS.map(n=><button type="button" key={n} onClick={()=>{setAmount(n);setMsg("")}} style={{padding:"14px 8px",borderRadius:14,border:amount===n?"2px solid #111":"1px solid #ddd",background:amount===n?"#f3f3f3":"#fff",fontWeight:800,cursor:"pointer"}}>₹{n}<small style={{display:"block",fontWeight:500,marginTop:3}}>{amount===n?"Selected":"Recharge"}</small></button>)}</div><div className="feeRow"><span><small>Selected Recharge</small><b>Wallet में ₹{amount}</b></span><strong>₹{amount}</strong></div>{payment.upiId?<a className="primaryAction payLink" href={upiLink}>📱 ₹{amount} UPI से Pay करें →</a>:<div className="notice">Admin ने अभी UPI ID set नहीं की है।</div>}{payment.qrUrl&&<div className="qr" style={{marginTop:12}}><img src={payment.qrUrl} alt="UPI QR" style={{maxWidth:250,width:"100%"}}/><small style={{display:"block",marginTop:8}}>QR scan के बाद ₹{amount} का payment करें और UTR नीचे डालें.</small></div>}<form onSubmit={recharge}><input className="adminInput" value={utr} onChange={e=>setUtr(e.target.value)} placeholder={"₹"+amount+" payment ka UTR / Transaction ID"}/><button className="primaryAction" disabled={saving}>{saving?"Sending...":"💳 ₹"+amount+" Recharge Request भेजें →"}</button></form></div>
